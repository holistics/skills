...
```aql
metric | previous(order: order_dimension, ...)
metric | previous(offset, order: order_dimension, ...)
metric | previous(offset, order: order_dimension, ..., reset: partition_dimension, ...)
metric | previous(offset, order: order_dimension, ..., partition: partition_dimension, ...)
```
...
### Examples
```aql
users | count(users.id) | previous(order: users | count(users.id) | desc())
users | count(users.id) | previous(2, order: users.created_at | month())
users | count(users.id) | previous(4, order: users.created_at | month(), reset: users.gender)
```
